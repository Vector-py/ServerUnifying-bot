...
try:
    ...
    async def help(ctx):
        ...
        emb.add_field(name=f"`{'!'}send_all_servers <text>` :", value="Отправить сообщение во все привязынные каналы",
                      inline=False)
        emb.add_field(name=f"`{'!'}delete <id сообщения>` :", value="Удалить сообщение из всех привязынных каналов",
                      inline=False)
        emb.add_field(name=f"`{'!'}edit <id сообщения> <text>` :",
    ...
    async def send_all_servers(ctx):
        chat = ctx.channel.id
        if chat in settings.chats:
            f = open('losm.txt', 'a')  # открытие файла для хранения списка сообщений
            text = ctx.message.content.replace('!send_all_servers \n', '').replace('!send_all_servers\n', '').replace(
                '!send_all_servers ', '').replace('!send_all_servers', '')
            f.write(f"{ctx.message.id} {chat} {ctx.message.guild.id} \n")  # запись сообщения отправителя в файл
            for i in settings.chats:  # проход по всем чатам
                if i == chat:
                    continue
                channel = bot.get_channel(i)
                message = await channel.send(text)
                f.write(f"{message.id} {i} {message.guild.id} \n")  # запись сообщения получателя в файл
            f.close()  # сохранение файла
        else:
    ...
    async def delete(ctx):
        ...
        if chat in settings.chats:
            ...
            with open('losm.txt', 'r') as f:
                lst = [row.strip().split() for row in f]
            # разграничение списка по дате отправления +-
            list_of_send_messages = [lst[i:i + len(settings.chats)] for i in
                                     range(0, len(lst), len(settings.chats))]
            ind = 1e18
            ...
            for i in range(len(list_of_send_messages)):
                if list_of_send_messages[i][0] == [str(id), str(chat), str(ctx.message.guild.id)]:
                    ind = i
                    break
            else:
                embed = Embed(title=f'**Ошибка**', color=randint(0, 16777215))
    ...
    async def edit(ctx):
        ...
        if chat in settings.chats:
            ...
            for i in list_of_send_messages[ind][1:]:
                try:
                    message = await bot.get_guild(int(i[2])).get_channel(int(i[1])).fetch_message(int(i[0]))
